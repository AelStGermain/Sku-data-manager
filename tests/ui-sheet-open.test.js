...
const products = JSON.parse(fs.readFileSync(new URL('../local_data/master_catalog.json', import.meta.url), 'utf8'))
...
function renderProduct(product) {
  ...
  const context = {
    ...
    App: { showToast() {} },
    DB: {
      getProduct: ean => String(product.ean) === String(ean) ? product : null,
      getHoldings: () => holdings,
      getVisperaBatch: () => [],
      computeCompleteness: () => 0,
      validateEAN: () => ({ valid: true, normalized: String(product.ean), legacy: false })
    }
  };
